...
$(function () {
    ...
    var ItemDetailTab = {
        ...
        BindItemRatingCriteria: function (msg) {
            if (msg.d.length > 0) {
                $.each(msg.d, function (index, item) {
                    ItemDetailTab.RatingCriteria(item);
                });
            } else {
                csscody.alert("<h2>" + getLocale(AspxItemDetailTab, "Information Alert") + "</h2><p>" + getLocale(AspxItemDetailTab, "No rating criteria are found!") + "</p>");
            }
        },
        ...
        BindItemBasicInfoByitemSKU: function (msg) {
            if (msg.d != null) {
                ...
                ItemDetailTab.BindItemsBasicInfo(msg.d);
                if (itemTypeId != 5) {
                    ItemDetailTab.GetCostVariantsByitemSKU(itemSKU);
                }
                ItemDetailTab.AddUpdateRecentlyViewedItem(itemSKU);
            }
        },
        ...
        BindItemFormAttributes: function (msg) {
            ...
            ItemDetailTab.CreateForm(msg.d, attributeSetId, itemTypeId, ItemDetailTab.vars.itemSKU);
            if (ItemDetailTab.vars.itemSKU.length > 0) {
                ItemDetailTab.BindDataInTab(ItemDetailTab.vars.itemSKU, attributeSetId, itemTypeId);
                ItemDetailTab.BindRatingReviewTab();
